package cubox.admin.main.service.vo;


public class LogBioRealInfoVO {

	private String fevttm;
	private String fgid;
	private String fuid;
	private String fdt;
	private String fsimg;
	private String fsextdata;
	private String fmatchvalue;
	private String fcimg;
	private String fsidx;
	private String faix;
	private String fcextdata;
	private String fsquality;
	private String fcquality;
	private String fpassvalue;
	private String fcaptime;
	private String fexttime;
	private String fmatchtime;
	private String fqstatus;
	private String fsstatus;
	private String fwvname;
	private String fwedt;
	private String fwsdt;
	private String fregdt;
	private String ffailcnt;
	private String ffailmaxcnt;
	
	
	public String getFevttm() {
		return fevttm;
	}
	public void setFevttm(String fevttm) {
		this.fevttm = fevttm;
	}
	public String getFgid() {
		return fgid;
	}
	public void setFgid(String fgid) {
		this.fgid = fgid;
	}
	public String getFuid() {
		return fuid;
	}
	public void setFuid(String fuid) {
		this.fuid = fuid;
	}
	public String getFdt() {
		return fdt;
	}
	public void setFdt(String fdt) {
		this.fdt = fdt;
	}
	public String getFsimg() {
		return fsimg;
	}
	public void setFsimg(String fsimg) {
		this.fsimg = fsimg;
	}
	public String getFsextdata() {
		return fsextdata;
	}
	public void setFsextdata(String fsextdata) {
		this.fsextdata = fsextdata;
	}
	public String getFmatchvalue() {
		return fmatchvalue;
	}
	public void setFmatchvalue(String fmatchvalue) {
		this.fmatchvalue = fmatchvalue;
	}
	public String getFcimg() {
		return fcimg;
	}
	public void setFcimg(String fcimg) {
		this.fcimg = fcimg;
	}
	public String getFsidx() {
		return fsidx;
	}
	public void setFsidx(String fsidx) {
		this.fsidx = fsidx;
	}
	public String getFaix() {
		return faix;
	}
	public void setFaix(String faix) {
		this.faix = faix;
	}
	public String getFcextdata() {
		return fcextdata;
	}
	public void setFcextdata(String fcextdata) {
		this.fcextdata = fcextdata;
	}
	public String getFsquality() {
		return fsquality;
	}
	public void setFsquality(String fsquality) {
		this.fsquality = fsquality;
	}
	public String getFcquality() {
		return fcquality;
	}
	public void setFcquality(String fcquality) {
		this.fcquality = fcquality;
	}
	public String getFpassvalue() {
		return fpassvalue;
	}
	public void setFpassvalue(String fpassvalue) {
		this.fpassvalue = fpassvalue;
	}
	public String getFcaptime() {
		return fcaptime;
	}
	public void setFcaptime(String fcaptime) {
		this.fcaptime = fcaptime;
	}
	public String getFexttime() {
		return fexttime;
	}
	public void setFexttime(String fexttime) {
		this.fexttime = fexttime;
	}
	public String getFmatchtime() {
		return fmatchtime;
	}
	public void setFmatchtime(String fmatchtime) {
		this.fmatchtime = fmatchtime;
	}
	public String getFqstatus() {
		return fqstatus;
	}
	public void setFqstatus(String fqstatus) {
		this.fqstatus = fqstatus;
	}
	public String getFsstatus() {
		return fsstatus;
	}
	public void setFsstatus(String fsstatus) {
		this.fsstatus = fsstatus;
	}
	public String getFwvname() {
		return fwvname;
	}
	public void setFwvname(String fwvname) {
		this.fwvname = fwvname;
	}
	public String getFwedt() {
		return fwedt;
	}
	public void setFwedt(String fwedt) {
		this.fwedt = fwedt;
	}
	public String getFwsdt() {
		return fwsdt;
	}
	public void setFwsdt(String fwsdt) {
		this.fwsdt = fwsdt;
	}
	public String getFregdt() {
		return fregdt;
	}
	public void setFregdt(String fregdt) {
		this.fregdt = fregdt;
	}
	public String getFfailcnt() {
		return ffailcnt;
	}
	public void setFfailcnt(String ffailcnt) {
		this.ffailcnt = ffailcnt;
	}
	public String getFfailmaxcnt() {
		return ffailmaxcnt;
	}
	public void setFfailmaxcnt(String ffailmaxcnt) {
		this.ffailmaxcnt = ffailmaxcnt;
	}
}
